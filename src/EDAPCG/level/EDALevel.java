...
public class EDALevel extends Level implements LevelInterface {
    ...
    public int placeBlockPowerUp(int x, int y) {
    	x = capX(x);
    	setBlock(x, y, EDALevel.BLOCK_POWERUP);
    	return x;
    }
    ...
    public int placeBlockCoin(int x, int y) {
    	x = capX(x);
    	y = capY(y);
    	setBlock(x, y, EDALevel.BLOCK_COIN);
    	return x;
    }
    ...
    public int placeBlockEmpty(int x, int y) {
    	x = capX(x);
    	y = capY(y);
    	setBlock(x, y, EDALevel.BLOCK_EMPTY);
    	return x;
    }
    
    public int placeCoin(int x, int y) {
    	x = capX(x);
    	setBlock(x, y, EDALevel.COIN);
    	return x;
    }
	...
	public int placePipe(int x, int y, int height) {
		...
		for (int i = 1; i < height; i++) {
			setBlock(x, y - i, EDALevel.TUBE_SIDE_LEFT);				
			setBlock(x + 1, y - i, EDALevel.TUBE_SIDE_RIGHT);							
		}
		
		setBlock(x, y - height, EDALevel.TUBE_TOP_LEFT);				
		setBlock(x + 1, y - height, EDALevel.TUBE_TOP_RIGHT);	
		return x+1;
	}
